...
import { Radio, Upload, X, Image as ImageIcon } from "lucide-react";
import { useState, useEffect } from "react";
import type { Receiver } from "@shared/schema";
...
interface ReceiverDialogProps {
  ...
  onSuccess?: (receiver: Receiver) => void;
}

export default function ReceiverDialog({ receiver, open, onOpenChange, onSuccess }: ReceiverDialogProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: ReceiverFormData) => {
      ...
      const res = await apiRequest("POST", "/api/electronics/receivers", { ...data, photoId });
      return res.json();
    },
    onSuccess: (newReceiver: Receiver) => {
      ...
      onOpenChange(false);
      if (onSuccess) onSuccess(newReceiver);
    },
    onError: (error: Error) => {
      setIsUploadingPhoto(false);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: ReceiverFormData) => {
      let photoId: number | null | undefined = receiver?.photoId ?? undefined;
      ...
      const res = await apiRequest("PUT", `/api/electronics/receivers/${receiver?.id}`, { ...data, photoId });
      return res.json();
    },
    onSuccess: (updatedReceiver: Receiver) => {
      queryClient.invalidateQueries({ queryKey: ["/api/electronics/receivers"] });
      toast({ title: "Receiver updated successfully" });
      onOpenChange(false);
      if (onSuccess) onSuccess(updatedReceiver);
    },
    onError: (error: Error) => {
      setIsUploadingPhoto(false);
      toast({ title: "Error", description: error.message, variant: "destructive" });
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-mono flex items-center gap-2">
            ...
            {receiver ? "Edit Receiver" : "Add Receiver"}
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField control={form.control} name="channels" render={({ field }) => (
                <FormItem><FormLabel>Channels</FormLabel><FormControl><Input type="number" {...field} value={field.value ?? ""} placeholder="e.g., 4" /></FormControl></FormItem>
              )} />
            </div>
            ...
            <Button type="submit" className="w-full" disabled={isPending}>
              {isPending ? "Saving..." : (receiver ? "Update Receiver" : "Add Receiver")}
            </Button>
          </form>
        </Form>
      </DialogContent>
